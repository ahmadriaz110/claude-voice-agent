...
| Say / do | What happens |
|---|---|
| Any message delivered to the session (`[voice]`, `[inbox]`) | Lands in the background: the composer takes the text over accessibility and a Return is posted to Claude's process, so whatever you are working in stays in front and your clipboard is untouched. When the window is covered by another app the renderer is throttled and the background write does not take, so the daemon brings Claude forward for the paste and hands focus straight back to the app you were in, within about a second (`BARGEIN_PASTE_FOREGROUND=1` forces the foreground paste always). A closed window is reopened the same way. Nothing is ever sent twice: the foreground paste replaces the composer's content |
